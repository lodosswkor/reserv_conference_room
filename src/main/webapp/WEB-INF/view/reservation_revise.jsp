...
<body>
	...
	<div align="center">
		...
		<div>
			<label>제목</label><br> <input type="text" id="title" name="title" value="${resResponse.title}"><br>
			<label>예약자</label><br> <input type="text" id="userName" name="userName" value="${resResponse.userName}"><br>
			<input type="hidden" id="id" name="id" value="${resResponse.id}">
		</div>

		<div class="dropdown">
			<label>회의실</label><br> 
				<select id="roomName" name="roomName">
					<option value="${resResponse.meetingRoom.name}" selected>${resResponse.meetingRoom.name}</option>
		...
		<div>
			<label>날짜</label><br> <input type="text" name="date" id="date" value="${resResponse.date}" readonly>
		</div>

		<div>
			<label>시작시간</label><br> 
			<select id="sTime" name="sTime">
				 <option value="${resResponse.sTime}" selected>${resResponse.sTime}</option>
				<%for (int i = 0; i< times.length; i++) {%>		
		...
		<div>
			...
			<select id="eTime" name="eTime">
				<option value="${resResponse.eTime}" selected>${resResponse.eTime}</option>
				<%for (int i = 0; i< times.length; i++) {%>		
						<option value=<%=times[i]%>><%=times[i]%></option>
				<%}%>
			</select>
